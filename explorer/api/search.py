...
def get_subtree():
    ...
    graph = flask.current_app.gs.get_subtree(node, int(max_depth),
                                             max_breadth=int(max_breadth), exclude_case_cache=exclude_case_cache)
...
def find():
    node = flask.request.args.get("node")
    nodes = flask.current_app.gs.find_matching(node)
